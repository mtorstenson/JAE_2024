%Sweeps through different migrant mortality rates \and outputs change in virulence
...
bfy = 0.8; %migrant fecundity rates 
mortality = [0.01:0.01:0.2];
h = length(mortality); 
count0 = 1;
...
bmx = mortality(floor(h/2)); %baseline mortality rate
...
delta_virulence_r_mat = nan(length(migrant_m),length(mortality)); %for residents
delta_virulence_m_mat = nan(length(migrant_m),length(mortality)); %for migrants

%initialize matrix of ratio of migrant:resident susceptibles at equilibrium
proportion_migrant_susceptibles_mat = nan(length(migrant_m),length(mortality));

%initialize matrix of difference between strategy that evolves alone in migrants
%and residents
difference_in_eq_virulence_mat = nan(length(migrant_m),length(mortality));

%initialize matrix of the weighted average of the optimal strategy
weighted_average_eq_virulence_mat = nan(length(migrant_m),length(mortality));

%initialize matrix of resident population size at equilibrium when run alone with
%infection
one_stability_resident_popsize_mat = nan(length(migrant_m),length(mortality));

%initialize matrix of migrant population size at equilibrium when run alone without
%infection
one_stability_migrant_popsize_mat = nan(length(migrant_m),length(mortality));

%initialize matrix of resident population size at equilibrium when run with
%migrant and infection
two_stability_resident_popsize_mat = nan(length(migrant_m),length(mortality));

%initialize matrix of migrant population size at equilibrium when run with
%resident and infection
two_stability_migrant_popsize_mat = nan(length(migrant_m),length(mortality));
...
for my = migrant_m
    ...
    for bmy = mortality
        ...
        bmy = mortality(lh_count); %migrant mortality rate
    ...
    one_stability_popsize = sum(one_stability_pop,2)*ones(1,length(mortality)); %resident alone
...
save output.mat
save(strcat(['2output_N=' num2str(N) '_mx=' num2str(mx)  '.mat']))